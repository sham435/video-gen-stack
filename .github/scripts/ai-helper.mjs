...
function getLatestRunLogs() {
  try {
    const runId = execSync(`gh run list --workflow="Publish News Video" --limit=1 --json=databaseId --jq='.[0].databaseId'`, {
      encoding: 'utf8', timeout: 15000, stdio: 'pipe',
    }).trim()
    if (!runId) return ''
    return execSync(`gh run view ${runId} --log`, { encoding: 'utf8', timeout: 30000, stdio: 'pipe' })
  } catch { return '' }
}
...
function createAutoPR(issues, fixes) {
  ...
  try {
    execSync(`git checkout -b ${branch} && git add -A`, { stdio: 'pipe', timeout: 10000 })
    const hasChanges = execSync('git diff --cached --stat', { encoding: 'utf8', timeout: 5000 }).trim()
    if (!hasChanges) {
      execSync(`git checkout main && git branch -D ${branch}`, { stdio: 'pipe' })
      return { pr: null, reason: 'no changes to commit' }
    }

    execSync(`git commit -m "fix: auto-apply detected pipeline fixes\n\n${issueList}\n${fixList}"`, { stdio: 'pipe', timeout: 10000 })
    execSync(`git push origin ${branch}`, { stdio: 'pipe', timeout: 15000 })
