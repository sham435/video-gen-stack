 ...
 *
 * Authoritative path: when the caller already built the central SEO object (via
 * src/publishing/seoMetadata.mjs, e.g. SocialPostGenerator / composer), its
 * `youtubeTags` projection is used AS-IS — the YouTube snippet.tags MUST equal
 * the central builder's output. `deriveYouTubeTags` remains only for
 * standalone/legacy callers that cannot reach the central builder yet.
 *
 * @param {object} args
 * @param {string|null} args.category pipeline category/niche key
 * @param {string[]} [args.articleTags]
 * @param {string} [args.brand]
 * @param {{youtubeTags?: string[]}|null} [args.seo] central SEO object (optional)
 * @returns {{tags:string[], categoryId:string}}
 */
export function buildYouTubeSEO(args = {}) {
  const tags = args.seo?.youtubeTags
    ? Array.from(args.seo.youtubeTags)
    : deriveYouTubeTags(args)
  return {
    tags,
