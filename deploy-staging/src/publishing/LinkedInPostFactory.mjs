...
// DEPRECATED compatibility projection — DO NOT extend.
//
// Production callers MUST pass the central SEO projection `linkedinHashtags`
// (from src/publishing/seoMetadata.mjs, '#'-prefixed, baseline-first, ≤5).
// This fallback only exists so older unit tests / one-off callers that still
// hand this factory a raw `hashtags` array keep working — it is NOT the
// normal path and must never be relied on by the pipeline.
function legacyHashtagProjection(rawHashtags) {
  const list = Array.isArray(rawHashtags) ? rawHashtags : []
...
export class VideoPostFormatter {
   ...
   * @param {string[]} video.hashtags (legacy only — prefer video.linkedinHashtags)
   * @param {string[]} video.linkedinHashtags (authoritative, from buildSeoMetadata)
  ...
  format(video) {
    ...
    // Authoritative: the central SEO LinkedIn projection. No re-slicing.
    const hashtags = video.linkedinHashtags || legacyHashtagProjection(video.hashtags)
...
export class ArticlePostFormatter {
   ...
   * @param {string[]} article.hashtags (legacy only — prefer article.linkedinHashtags)
   * @param {string[]} article.linkedinHashtags (authoritative, from buildSeoMetadata)
  ...
  format(article) {
    ...
    // Authoritative: the central SEO LinkedIn projection. No re-slicing.
    const hashtags = article.linkedinHashtags || legacyHashtagProjection(article.hashtags)
