import { buildSeoMetadata } from './seoMetadata.mjs'
...
//
// SEO CONTRACT: this class is the AUTHORITATIVE producer of the central SEO
// object. It calls buildSeoMetadata() ONCE and attaches the projections
// (hashtags/youtubeTags/linkedinHashtags) to the built post. Platform builders
// below only FORMAT what the central builder decided — nobody re-derives tags.
...
export class SocialPostGenerator {
  build(video) {
    ...
    // ONE central SEO object for this post — every platform consumes it.
    const seo = buildSeoMetadata({
      ...
      keywords: video.keywords || video.topicKeywords || [],
    })

    // Platform-ready projections: display tags are '#'-prefixed (LinkedIn body
    // and community text render hashtags WITH '#'); youtubeTags stays bare.
    const hashtags = seo.linkedinHashtags
    const youtubeTags = seo.youtubeTags
  ...
  linkedinPost({ title, hook, summary, videoUrl, youtubeShortsUrl, hashtags, category }) {
    ...
    return {
      ...
      media: { url: ytLink },
      // Already the central LinkedIn projection (≤5, baseline first) — do NOT re-slice.
      hashtags,
    }
  }
