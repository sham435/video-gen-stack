  ...
  const runFull = async () => {
    ...
    for (const rawArticle of articles) {
      ...
      job.onStage('UPLOAD', async (ctx) => {
        ...
        const { buildSeoMetadata } = await import('../src/publishing/seoMetadata.mjs')
        ...
        // ONE central SEO object for this story. Every publisher downstream
        // (YouTube snippet.tags, LinkedIn native video, promo posts) consumes
        // these projections — nobody may rebuild title/tags/hashtags locally.
        const seo = buildSeoMetadata({
          headline: article.title,
          title: article.title,
          summary: article.description,
          category: seoCategory || 'tech',
          hashtags: Array.isArray(article.tags) ? article.tags : [],
          keywords: article.keywords || [],
        })
        const hashtags = seo.hashtags.map((t) => `#${t}`).join(' ')
        // YouTube SEO: consume the central youtubeTags projection. `seo` is
        // threaded so the YouTube snippet.tags ALWAYS equal the central output
        // (deriveYouTubeTags stays only for standalone callers of YouTubeSEO).
        ...
        return { uploadTitle, hashtags, seo, nicheDecision, ...result }
      })
      ...
      job.onStage('PUBLISH', async (ctx) => {
        const { engine } = ctx.results.RENDER
        const plan = ctx.results.DISCOVER?.plan
        const { videoId, uploadTitle, hashtags, seo, nicheDecision } = ctx.results.UPLOAD
          ...
          try {
            ...
            const results = await factory.publishVideo(
              ...
              buffer,
              {
                ...
                // Authoritative LinkedIn projection from the central builder.
